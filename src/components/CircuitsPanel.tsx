  ...
  return (
    <div className="absolute right-4 top-4 w-64 rounded-xl border border-[color:var(--hairline)] bg-[color:var(--panel-bg-solid)] backdrop-blur-xl p-3.5 shadow-[0_0_30px_rgba(0,0,0,0.25)]">
      ...
      <div className="flex flex-col gap-1.5 mb-3 max-h-32 overflow-y-auto">
        {lvl.circuits.map((c) => {
          const def = TYPE_DEFS.find((t) => t.key === c.type)!
          return (
            <div key={c.id} className="flex items-center gap-2 text-[11.5px]">
              <div className="w-2 h-2 rounded-full flex-none" style={{ background: def.color }} />
      ...
      <div className="flex flex-col gap-1.5 mb-1">
        ...
        <button onClick={handleAdd} className="rounded-[8px] py-1.5 text-[11px] font-semibold text-[#0a0a10]" style={{ background: 'var(--layer-electrica)' }}>+ Agregar circuito</button>
      </div>
